...
func (c *Route53Controller) processIngressDelete(ctx context.Context, key string) (reconcile.Result, error) {
	...
	err = cloud.CleanupRecordSet(ctx, c.apiHost, "ingress", ns, name)
	if err != nil {
		klog.Error(err)
...
func (c *Route53Controller) processIngressCreateOrUpdate(ctx context.Context, obj runtime.Object) (reconcile.Result, error) {
	...
	if !ok {
		cloud := cloudaws.NewAWS("us-west-2")
		err := cloud.CleanupRecordSet(ctx, c.apiHost, "ingress", ingress.Namespace, ingress.Name)
		if err != nil {
			klog.Error(err)
			return reconcile.Result{}, err
		}
	...
	for i := range ingress.Status.LoadBalancer.Ingress {
		...
		case "aws":
			...
			created, retryAfter, err := cloud.EnsureRoute53ForIngress(ctx, ingress, &lbIngress, hostnames, c.apiHost)
			if err != nil {
				return reconcile.Result{}, err
			}
			if retryAfter > 0 {
