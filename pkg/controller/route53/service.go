...
func (c *Route53Controller) processServiceDelete(ctx context.Context, key string) (reconcile.Result, error) {
	...
	err = cloud.CleanupRecordSet(ctx, c.apiHost, "service", ns, name)
	if err != nil {
		klog.Error(err)
...
func (c *Route53Controller) processServiceCreateOrUpdate(ctx context.Context, obj runtime.Object) (reconcile.Result, error) {
	...
	if !ok {
		cloud := cloudaws.NewAWS("us-west-2")
		err := cloud.CleanupRecordSet(ctx, c.apiHost, "service", svc.Namespace, svc.Name)
		if err != nil {
			klog.Error(err)
			return reconcile.Result{}, err
		}
	...
	for i := range svc.Status.LoadBalancer.Ingress {
		...
		case "aws":
			...
			created, retryAfter, err := cloud.EnsureRoute53ForService(ctx, svc, &lbIngress, hostnames, c.apiHost)
			if err != nil {
				return reconcile.Result{}, err
			}
			if retryAfter > 0 {
