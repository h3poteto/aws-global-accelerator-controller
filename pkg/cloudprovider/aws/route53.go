...
func Route53OwnerValue(apiHost, resource, ns, name string) string {
	return "\"heritage=aws-global-accelerator-controller,cluster=" + apiHost + "," + resource + "/" + ns + "/" + name + "\""
}
...
func (a *AWS) EnsureRoute53ForService(
	...
	return a.ensureRoute53(ctx, lbIngress, hostnames, apiHost, "service", svc.Namespace, svc.Name)
}
...
func (a *AWS) EnsureRoute53ForIngress(
	...
	return a.ensureRoute53(ctx, lbIngress, hostnames, apiHost, "ingress", ingress.Namespace, ingress.Name)
}
...
func (a *AWS) ensureRoute53(
	...
	lbIngress *corev1.LoadBalancerIngress,
	hostnames []string,
	apiHost, resource, ns, name string,
) (bool, time.Duration, error) {
...
HOSTNAMES:
	for _, hostname := range hostnames {
		...
		records, err := a.FindOwneredARecordSets(ctx, hostedZone, Route53OwnerValue(apiHost, resource, ns, name))
		if err != nil {
			klog.Error(err)
			return false, 0, err
		}
		...
		if record == nil {
			...
			err = a.createMetadataRecordSet(ctx, hostedZone, hostname, apiHost, resource, ns, name)
			if err != nil {
				klog.Error(err)
				return false, 0, err
			}
...
func (a *AWS) CleanupRecordSet(ctx context.Context, apiHost, resource, ns, name string) error {
	...
	for _, zone := range zones {
		records, err := a.FindOwneredARecordSets(ctx, zone, Route53OwnerValue(apiHost, resource, ns, name))
		if err != nil {
			klog.Error(err)
			return err
		}
		...
		records, err = a.findOwneredMetadataRecordSets(ctx, zone, Route53OwnerValue(apiHost, resource, ns, name))
		if err != nil {
			klog.Error(err)
			return err
		}
...
func (a *AWS) createMetadataRecordSet(ctx context.Context, hostedZone *route53.HostedZone, hostname, apiHost, resource, ns, name string) error {
	input := &route53.ChangeResourceRecordSetsInput{
		HostedZoneId: hostedZone.Id,
		ChangeBatch: &route53.ChangeBatch{
			Changes: []*route53.Change{
					...
					ResourceRecordSet: &route53.ResourceRecordSet{
						...
						ResourceRecords: []*route53.ResourceRecord{
							&route53.ResourceRecord{
								Value: aws.String(Route53OwnerValue(apiHost, resource, ns, name)),
							},
						},
					},
				},
